...
2.拷贝XTA_W32\Api\XTP_vX.X.X.X_XXXXXXXX目录（X.X.X.X_XXXXXXXX为版本号）下的所有dll文件到可执行文件目录，
...
public:
	...
	Config()
	{
        ...
        ClientID = "1";//可选，同一账户多点登录时标识不同的登录点，范围：0~255
...
{
	printf("  Tick,%s %s, HighestPrice=%g, LowestPrice=%g, BidPrice0=%g, BidVolume0=%lld, AskPrice0=%g, AskVolume0=%lld, LastPrice=%g, TotalVolume=%lld, TradingTime=%s\n",
...
{
	printf("  ProductType=%d, Ref=%s, ID=%s, InstID=%s %s, Price=%g, Volume=%lld, NoTradedVolume=%lld, Direction=%d, PriceCond=%d, SessionID=%lld, Status=%d, Msg=%s, %s\n",
		order.ProductType,
...
{
	printf("  ID=%s, OrderRef=%s, InstID=%s %s, Price=%g, Volume=%lld, Direction=%d, %s\n",
...
{
	printf("  InstID=%s %s, Direction=%d, PositionTotal=%lld, PositionSellable=%lld, PosYesterday=%lld, AvgPrice=%g\n",
		pos.ExchangeID.c_str(), pos.InstrumentID.c_str(), pos.Direction,
...
void MarketTest()
{
	...
	int err = -1;
#ifdef _DEBUG
	market = XFinApi_CreateMarketApi("XTA_W32/Api/XTP_v1.1.18.13_20180516/XFinApi.XTPTradeApid.dll", &err);
#else
	market = XFinApi_CreateMarketApi("XTA_W32/Api/XTP_v1.1.18.13_20180516/XFinApi.XTPTradeApi.dll", &err);
#endif
...
void TradeTest()
{
	...
	int err = -1;
#ifdef _DEBUG
	trade = XFinApi_CreateTradeApi("XTA_W32/Api/XTP_v1.1.18.13_20180516/XFinApi.XTPTradeApid.dll", &err);
#else
	trade = XFinApi_CreateTradeApi("XTA_W32/Api/XTP_v1.1.18.13_20180516/XFinApi.XTPTradeApi.dll", &err);
#endif
	if (err || !trade)
	{
